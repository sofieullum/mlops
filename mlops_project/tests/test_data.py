...
def test_my_dataset():
    ...
    for dataset in [train, test]:
        for x,y in dataset:
            assert x.shape == torch.Size([1,28,28]), f"Expected x to have shape torch.Size([1,28,28]), but got {x.shape}"
            assert y in range (10), f"Expected y to be in range 10, but got {y}"
    train_targets = torch.unique(train.tensors[1])
    assert (train_targets == torch.arange(0,10)).all(), f"Expected training targets to be in range 1-10, got {train_targets}"
    test_targets = torch.unique(test.tensors[1])
    assert (test_targets == torch.arange(0,10)).all(), f"Expected training targets to be in range 1-10, got {test_targets}"
